...
assert.match(script, /const showPrintConfirmationFallback = \([\s\S]*if \(!printInProgress \|\| !confirmationNode\) return;[\s\S]*confirmationNode\.hidden = false;[\s\S]*scrollIntoView\(\{ behavior: 'smooth', block: 'center', inline: 'center' \}\)/, 'confirmation should be print-gated and automatically centered in the viewport');
