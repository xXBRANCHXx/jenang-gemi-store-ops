...
assert.match(script, /currentOrder\.status = 'FULFILLED';[\s\S]*currentOrder\.fulfillmentStatus = 'FULFILLED'/, 'a server-acknowledged completion should update the local order cache');
...
assert.match(storeHome, /const serverAcknowledged = Boolean\(printedLabel && printedLabel\.serverAcknowledgedAt\);[\s\S]*status: 'FULFILLED',[\s\S]*fulfillmentStatus: 'FULFILLED'/, 'refresh merging must hide a local printed order only after the shared server acknowledged it');
assert.doesNotMatch(storeHome, /const locallyPrinted = Boolean\(printedLabel && printedLabel\.printedAt\)/, 'a device-local print marker must never hide an unacknowledged order');
...
const closeFlow = script.match(/const closeConfirmedPrintTab = async \(\) => \{[\s\S]*?\n  \};\n\n  const armAutomaticPrintConfirmation/);
assert.ok(closeFlow, 'the direct confirmation-close flow should be present');
assert.match(closeFlow[0], /await submitPrintCompletion\(\)[\s\S]*markPrinted\(serverAcknowledgedAt\);[\s\S]*window\.close\(\);/, 'the red confirmation button must wait for shared fulfillment acknowledgement before removing the card and closing its tab');
assert.match(closeFlow[0], /catch \(error\)[\s\S]*This order remains listed on every device; press Printed successfully to retry\./, 'a rejected completion must stay visible and offer a clear retry');
...
assert.match(script, /const submitPrintCompletion = async \(\) => \{[\s\S]*persistPendingCompletion\(payload\)[\s\S]*const response = await fetch/, 'completion must be durably queued before waiting for the authoritative server response');
assert.match(script, /if \(!response\.ok \|\| responsePayload\.ok === false\)[\s\S]*throw new Error[\s\S]*removeAcknowledgedCompletion\(key\)/, 'the print page must keep failed completions queued and clear only acknowledged work');
assert.match(storeHome, /const flushPendingOrderCompletions = \(\) => \{[\s\S]*fetch\(ordersEndpoint[\s\S]*if \(!response\.ok \|\| responsePayload\.ok === false\) continue;[\s\S]*removeAcknowledgedCompletion/, 'the dashboard must retain completion actions until the server acknowledges them');
assert.match(script, /orderActionPayload\('fulfill_order',[\s\S]*fulfilled_at:[\s\S]*items: completionItems\(\)[\s\S]*customer_name:/, 'completion must send every ordered product and the customer identifier');
assert.match(fulfillmentRuntime, /customer_name VARCHAR\(160\)[\s\S]*function jg_store_ops_fulfillment_mark_fulfilled[\s\S]*customer_name = CASE/, 'completion must persist the customer identifier with the processed order');
...
assert.match(ordersApi, /Marketplace processed callback failed:[\s\S]*throw new RuntimeException\('Marketplace did not acknowledge the processed status\. Store Ops will retry\.'/s, 'the current endpoint must not acknowledge fulfillment while is_listed may still be true upstream');
assert.match(legacyOrdersApi, /Marketplace processed callback failed:[\s\S]*throw new RuntimeException\('Marketplace did not acknowledge the processed status\. Store Ops will retry\.'/s, 'the compatible endpoint must not acknowledge fulfillment while is_listed may still be true upstream');
