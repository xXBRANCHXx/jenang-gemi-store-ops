...
require __DIR__ . '/login-identity.php';
...
$selectedEmployeeId = (string) ($employeeProfiles[0]['id'] ?? '');
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $submittedCode = (string) ($_POST['admin_code'] ?? '');
    $submittedEmployeeId = (string) ($_POST['employee_id'] ?? '');
    if ($employeeProfiles !== [] && array_key_exists('username', $_POST)) {
        // Autofill can choose a different account from the initially selected tile.
        $submittedEmployeeId = jg_store_login_employee_id($employeeProfiles, (string) $_POST['username']);
    }
    $selectedEmployeeId = $submittedEmployeeId;
    $loggedIn = $employeeProfiles !== []
        ? ($submittedEmployeeId !== '' && jg_admin_attempt_employee_login($submittedEmployeeId, $submittedCode))
...
<body class="admin-body is-login">
        ...
        <section class="admin-login-card">
            ...
            <form method="post" class="admin-login-form" autocomplete="on" data-employee-login-form>
                ...
                <?php if ($employeeProfiles !== []): ?>
                    <div class="admin-employee-login-grid" role="radiogroup" aria-label="Employee profile">
                        <?php foreach ($employeeProfiles as $index => $employee): ?>
                            ...
                            $isSelected = $employeeId === $selectedEmployeeId;
                            ?>
                            <button
                                type="button"
                                class="admin-employee-tile<?php echo $isSelected ? ' is-active' : ''; ?>"
                    ...
                    <input type="hidden" name="employee_id" data-employee-id-input value="<?php echo htmlspecialchars($selectedEmployeeId, ENT_QUOTES, 'UTF-8'); ?>">
                    <label for="username">Account</label>
                    <input id="username" name="username" type="text" data-employee-username-input autocomplete="username" autocapitalize="none" spellcheck="false" required value="<?php echo htmlspecialchars($selectedEmployeeId, ENT_QUOTES, 'UTF-8'); ?>">
                ...
                <?php else: ?>
                    ...
                    <label for="username">Account</label>
                    <input id="username" name="username" type="text" autocomplete="username" value="Store Admin" readonly>
    ...
    <script src="./login.js?v=<?php echo urlencode((string) @filemtime(__DIR__ . '/login.js')); ?>" defer></script>
</body>
</html>
