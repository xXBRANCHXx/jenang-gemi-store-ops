...
function jg_store_ops_employees_request_json(): array
{
...
function jg_store_ops_employees_normalize_id(string $value): string
{
    $normalized = strtolower(trim((string) preg_replace('/[^a-z0-9._-]+/i', '-', $value)), '.-_');
    return substr($normalized, 0, 64);
}

function jg_store_ops_employees_fetch(PDO $pdo): array
{
    $rows = $pdo->query(
...
function jg_store_ops_employees_response(PDO $pdo, array $extra = []): void
{
    echo json_encode(array_merge([
        'ok' => true,
        'employees' => jg_store_ops_employees_fetch($pdo),
        'current_employee' => [
...
if (!jg_admin_can_manage_employee_profiles()) {
    jg_store_ops_employees_fail('Only an admin employee can manage Store Ops profiles.', 403);
}
...
if ($method === 'GET') {
    jg_store_ops_employees_response($pdo);
}

if ($method !== 'POST') {
    jg_store_ops_employees_fail('Method not allowed.', 405);
}

$payload = jg_store_ops_employees_request_json();
$action = trim((string) ($payload['action'] ?? 'save_employee'));

if ($action !== 'save_employee') {
    jg_store_ops_employees_fail('Unknown employee action.', 400);
}

$displayName = trim((string) ($payload['display_name'] ?? ''));
$employeeId = jg_store_ops_employees_normalize_id((string) ($payload['id'] ?? ''));
$pin = trim((string) ($payload['pin'] ?? ''));
$active = !empty($payload['active']);

if ($displayName === '') {
    jg_store_ops_employees_fail('Employee display name is required.');
}

if ($employeeId === '') {
    $employeeId = jg_store_ops_employees_normalize_id($displayName);
}

if ($employeeId === '') {
    jg_store_ops_employees_fail('Employee ID is required.');
}

try {
    $stmt = $pdo->prepare('SELECT id, active FROM store_ops_employees WHERE id = :id LIMIT 1');
    $stmt->execute([':id' => $employeeId]);
    $existing = $stmt->fetch();
    $isExisting = is_array($existing);

    if (!$active && hash_equals($employeeId, jg_admin_current_employee_id())) {
        jg_store_ops_employees_fail('You cannot deactivate the employee profile you are using.');
    }

    if (!$isExisting && $pin === '') {
        jg_store_ops_employees_fail('PIN or password is required for a new employee.');
    }

    if ($pin !== '' && strlen($pin) < 4) {
        jg_store_ops_employees_fail('PIN or password must be at least 4 characters.');
    }

    if ($pin !== '' && strlen($pin) > 128) {
        jg_store_ops_employees_fail('PIN or password is too long.');
    }

    if ($isExisting) {
        if ($pin !== '') {
            $stmt = $pdo->prepare(
                'UPDATE store_ops_employees
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO store_ops_employees (id, display_name, pin_hash, active, created_at, updated_at)
