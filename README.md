...
- Partner orders remain visible after acceptance and throughout scanning. Clicking the shipping-label print action opens the system print dialog immediately from the operator's click and begins the server update. The loaded-label page always offers a successful-print confirmation, including after refreshing an older print tab, so an operator can recover without printing a duplicate label. Confirmation removes the order from Listed immediately, waits for the idempotent server update, and then closes the tab. Slow updates time out with a retry instead of leaving the confirmation controls disabled indefinitely.
