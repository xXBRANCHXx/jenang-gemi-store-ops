...
While Big Set is OFF (or its local state is unavailable), Store Ops does not fetch, display, cache-fallback, print, claim, scan, or complete any Shopee/TikTok order. Marketplace rows become eligible only after the irreversible activation projection is stored locally; unrelated partner and in-store workflows remain available.
