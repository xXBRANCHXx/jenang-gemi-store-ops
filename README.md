...
- Partner orders remain visible after acceptance and throughout scanning. Clicking the shipping-label print action opens the system print dialog immediately from the operator's click, then completes the order and removes it from the Store Ops list. Store Ops closes the print tab only after the PDF frame or browser reports that printing finished and the server update succeeds; manual confirmation appears as a delayed fallback and safely retries a failed update.
