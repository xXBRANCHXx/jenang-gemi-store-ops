...
- Partner orders remain visible after acceptance and throughout scanning. Clicking the shipping-label print action opens the system print dialog immediately from the operator's click; when that dialog closes, the order is removed from Listed immediately while any queued scan history and the idempotent server completion save in the background. The loaded-label page always offers a successful-print confirmation, including after refreshing an older print tab, so an operator can recover without printing a duplicate label. Confirmation finishes the server update and closes the tab. Slow updates time out with a retry instead of leaving the confirmation controls disabled indefinitely.
