...
- `/order-records/` includes a Branch-only history repair for a missing completed record. It requires the exact Order ID, a fresh Branch Login passcode, and an existing account-scoped `deducted` inventory ledger entry. The repair is idempotent and only restores the local `fulfill` history event and missing display snapshot; it never changes stock, marketplace state, or the Listed queue.
