...
- Partner orders remain visible after acceptance and throughout scanning. Clicking the shipping-label print action completes the order and removes it from the Store Ops list before the system print dialog opens. Store Ops closes the print tab automatically when the PDF frame or browser reports that printing finished; manual confirmation appears only as a delayed fallback and affects only tab cleanup.
